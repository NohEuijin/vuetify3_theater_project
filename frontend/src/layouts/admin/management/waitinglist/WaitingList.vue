...
>
  ...
  <tbody>
    ...
    v-for="(users, index) of waintAdminList" :key="index"
    >
      <td>
        <input
...
methods: {
  ...
  async getWaitAdminList(){
    ...
    await this.$store.dispatch('waitingAdminList', form)
    .then((res) => {
      console.log(res);
      this.total_page = res.usersConnection.aggregate.count;
      this.waintAdminList = res.users;
    })
  ...
  if (isChecked) {
    this.waintAdminList.forEach(user => {
...
async completAdmin(index){
  // console.log(this.waintAdminList[index].id)
  const userId = this.waintAdminList[index].id
  const username = this.waintAdminList[index].name
  const adminconfirm = confirm(username + "님을 관리자로 승인 하시겠습니까?")
  if(adminconfirm){
    const form = {
    id : userId,
    role : 3
  ...
  // console.log(form)
  await this.$store.dispatch("updateAdmin",form);
  confirm("관리자 승인이 완료 되었습니다!")
  this.getWaitAdminList();
  }
  return;
},
